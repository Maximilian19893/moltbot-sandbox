...
# 4. Injects CLAUDE_SETUP_TOKEN for subscription auth (custom addition)
...
// Claude subscription auth: set model to Anthropic default
// (auth-profiles.json injection happens below in the bash section)
if (process.env.CLAUDE_SETUP_TOKEN) {
    ...
    // Remove the placeholder API key from onboard if present
    if (config.models?.providers?.anthropic?.apiKey === 'placeholder-for-setup-token') {
...
# This enables Claude Max/Pro subscription auth without an API key
if [ -n "$CLAUDE_SETUP_TOKEN" ]; then
    ...
    if echo "$CLAUDE_SETUP_TOKEN" | openclaw models auth paste-token --provider anthropic 2>&1; then
        echo "Setup-token injected via CLI (openclaw models auth paste-token)"
    else
