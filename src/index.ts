...
function validateRequiredEnv(env: MoltbotEnv): string[] {
  ...
  if (!hasCloudflareGateway && !hasLegacyGateway && !hasAnthropicKey && !hasOpenAIKey && !hasSetupToken) {
    missing.push(
...
async function scheduled(
  ...
  // Skip sync if gateway hasn't started yet (prevents race condition on cold start)
